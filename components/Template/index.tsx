...
const codeValue = `
/**
 * Must return a Template function
 */
...
function Template({ data }: { data: DataScheme }) {
    return (
        <>
            <Me />
...
function findStyleSheetByTitle(title: string) {
    const arr = Array.from(document.styleSheets)
    return arr.find(c => c.title == title)
}
function changeStyleSheetSelectors(s: CSSStyleSheet | undefined, fn: (s:string) => string)
{
    if (!s) return
...
function setStyleSheetDisabled(disabled: boolean) {
    let styleSheet = findStyleSheetByTitle('customResumeCss')
...
function ResumeUI({ mstate, emit }: { mstate: StateValue, emit: any }) {
    ...
    const [code, setCode] = useState({ js: codeValue, css: '' });
    const { onPrevButtonClick, onNextButtonClick } = useEmblaButtons(emblaApi)

    async function applyTemplate() {
        const Babel = await import('@babel/standalone')
        const tcode = Babel.transform(
            `getComp = (React) => { ${code.js}; return { Template } }`,
            { presets: ['env', 'react'] }
        ).code;
        let getComp: (r: any) => { Template: TemplateType } = (_) => ({ Template })

        if (tcode) {
            // setStyleSheetDisabled(true)
            eval(tcode);
            setTemplate(getComp(React))
            if (styleRef.current) styleRef.current.innerHTML = code.css
            changeStyleSheetSelectors(
                findStyleSheetByTitle('customResumeCss'),
                s => `#resumeUI ${s}`
            )
            // setStyleSheetDisabled(false)
        }
    }
    ...
    return (
        <div className="flex gap-4 min-h-screen min-w-screen p-2 print:p-0 print:block">
            ...
            <div className="flex flex-grow">
                <div className="mx-auto flex gap-4 items-start">
                    <div key="page" id="resumeUI" className={`my-auto box-content border ${alice.variable}`}>
                        ...
                        <div className="relative self-stretch w-[calc(12rem/1.414)] print:hidden">
                            <div className="absolute top-0 left-0 h-full w-full overflow-hidden" ref={emblaRef}>
                                <div className="flex flex-col gap-4 justify-centerf h-full max-h-full">
                                    <div className="min-h-0 basis-[4rem] flex-grow-0 flex-shrink-0"></div>
                                    <div className="min-h-0 basis-[12rem] flex-grow-0 flex-shrink-0 w-[calc(12rem/1.414)] rounded-md bg-gray-500">Slide 1</div>
                                    <div className="min-h-0 basis-[12rem] flex-grow-0 flex-shrink-0 w-[calc(12rem/1.414)] rounded-md bg-gray-500">Slide 2</div>
                                    <div className="min-h-0 basis-[12rem] flex-grow-0 flex-shrink-0 w-[calc(12rem/1.414)] rounded-md bg-gray-500">Slide 3</div>
                                    <div className="min-h-0 basis-[12rem] flex-grow-0 flex-shrink-0 w-[calc(12rem/1.414)] rounded-md bg-gray-500">Slide 4</div>
                                    <div className="min-h-0 basis-[12rem] flex-grow-0 flex-shrink-0 w-[calc(12rem/1.414)] rounded-md bg-gray-500">Slide 5</div>
                                    <div className="min-h-0 basis-[12rem] flex-grow-0 flex-shrink-0 w-[calc(12rem/1.414)] rounded-md bg-gray-500">Slide 6</div>
                                    <div className="min-h-0 basis-[12rem] flex-grow-0 flex-shrink-0 w-[calc(12rem/1.414)] rounded-md bg-gray-500">Slide 7</div>
                                    <div className="min-h-0 basis-[12rem] flex-grow-0 flex-shrink-0 w-[calc(12rem/1.414)] rounded-md bg-gray-500">Slide 8</div>
                                    <div className="min-h-0 basis-[12rem] flex-grow-0 flex-shrink-0 w-[calc(12rem/1.414)] rounded-md bg-gray-500 mb-4">Slide 9</div>
                                    <div className="min-h-0 basis-[3.5rem] flex-grow-0 flex-shrink-0"></div>
                                </div>
                                <button
...
})
{
    ...
    return (
        <div className="self-stretch flex flex-col gap-0 w-[60rem] flex-grow-0">
            ...
            <ReactCodeMirror
                className="h-full flex-grow text-xl"
