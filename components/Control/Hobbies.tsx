...
function HobbyForm({ hobby, onUpdate }: {
        ...
        <section className="flex flex-col gap-2 p-3 bg-white rounded-lg shadow-md">
            <header className="flex justify-between items-start">
                <div className="flex flex-col">
                    <h2 className="text-xl font-medium">{hobby.name}</h2>
                </div>
                <Switcher
                    initial={hobby.enabled}
                    onChange={enabled => emit?.({type:'hobby.update', id: hobby.name, value: {enabled}})}
                    size="sm"
                />
            </header>
