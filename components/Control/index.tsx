...
export default function Panel() {
    ...
    return (
        <div className="fixed bottom-4 left-4 sm:bottom-8 sm:left-8 flex flex-col-reverse sm:flex-row gap-7 items-start sm:items-end print:hidden">
            ...
            {/* create template */}
            <Button onClick={() => emit?.({ type: 'createTemplate' })} bgClass='bg-blue-700' text="create item">
...
function Button({ children, onClick, className, bgClass, text, stroke }: {
    ...
    return (
        <button onClick={onClick} className={`group relative h-16 w-16  ${stroke ? "stroke-2 stroke-[#eee] fill-none" : "fill-[#eee] stroke-none"} flex ${className}`} >
...
const Category = ({ cat, show, onClick, order }: {
    cat: Category, order: number, 
    show: boolean, onClick: () => void
}) => (
    <li
        className={`group relative font-light cursor-pointer transition-all duration-[250ms]  ${show ? "opacity-0 delay-0" : "delay-[var(--delay)]"}`}
        onClick={onClick}
